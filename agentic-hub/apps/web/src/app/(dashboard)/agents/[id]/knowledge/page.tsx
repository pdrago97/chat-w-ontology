...
import { ArrowLeft, Bot, MessageSquare, Database, Plug, Settings } from 'lucide-react';
import { AgentKnowledgeManager } from './knowledge-manager';
...
export default async function AgentKnowledgePage({ params }: Props) {
  ...
  const tabs = [
    { name: 'Configurações', href: `/agents/${id}`, icon: Settings },
    { name: 'Knowledge Base', href: `/agents/${id}/knowledge`, icon: Database, active: true },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Content */}
      <AgentKnowledgeManager agentId={agent.id} organizationId={session.organizationId} />
    </div>
  );
}
